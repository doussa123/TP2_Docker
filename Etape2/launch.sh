...
docker container run -d --name data --network mon_network -e MYSQL_ROOT_PASSWORD=root -e MYSQL_DATABASE=testdb -e MYSQL_USER=user -e MYSQL_PASSWORD=password -v "$(pwd)/init.sql:/docker-entrypoint-initdb.d/init.sql" mysql:latest
